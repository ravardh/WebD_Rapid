...
const SavedJobs = () => {
  ...
  const handleApplyJob = async (applicationId, jobId) => {
    try {
      // Use the new backend route that handles both unsaving and applying
      //convert the call in query

      const res = await axios.patch(`/user/applysaved?applicationId=${applicationId}`);
